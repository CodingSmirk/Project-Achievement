...
class CaptureWorker(QThread):
    def __init__(self, sock, cap, label):
        ...
        print("Start capture worker thread")
    ...
    def run(self):
        while self.trig:
            ...
            try:
                ...
                #print(packet.hex(), packet[-6:][:2], type(packet[-6:][:2].hex()))

                if packet[-6:][:2] == PING_INDICATOR :
                    # print(packet[-4:].hex(), int.from_bytes(packet[-4:], byteorder="big", signed=False))

                    # delay 계산을 위한 time data 추가
                    ...
                    #print(packet[-4:].hex(),int.from_bytes(packet[-4:],byteorder="big"))
...
class SenderWindow(QWidget):
    def __init__(self):
        ...
        while True:
            try:
                ...
                print("Sender successfully connected to server")
                break
            except:
                print(traceback.format_exc())
        ...
        self.type_combo.addItem("Real Time Camera")
    ...
    def play_send_video(self):
        ...
        if self.type_combo.currentText() == "Real Time Camera":
            try:
                self.video_cap = cv2.VideoCapture(cv2.CAP_DSHOW+1)
            except Exception as e:
                print(traceback.format_exc())
                return
        elif self.type_combo.currentText() == "Saved Video":
            ...
            print("Error of [self.type_combo.currentText]")
            return
    ...
    def pause_video(self):
        print("Sender stop sending Data")
        self.cap_th.stop()
        self.ping_th.stop()
