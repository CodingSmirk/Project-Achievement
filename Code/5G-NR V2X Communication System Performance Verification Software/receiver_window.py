        ...
        while self.trig:
            ...
            if num_header > 0:
                try:
                    ...
                    wr = csv.writer(f)
                    # header_list = ['No.', 'magic_num', 'ver', 'psid', 'e_v2x_comm_type', 'e_payload_type', 'freq',
                    #                'rssi', 'is_signed', 'reserved', 'peer_l2id','reserved2', 'reserved3', 'crc',
                    #                'length', 'eDeviceType', 'eTeleCommType', 'unDeviceId', 'ulTimeStamp', 'Unknown',
                    ...
                    while True:
                        try:
                            ...
                            # V2X_RxPDU (length = 38)
                            # magic_num = header_log[0:2]
                            # ver = struct.unpack(">H", header_log[2:4])[0]
                            # psid = struct.unpack(">i", header_log[4:8])[0]
                            # e_v2x_comm_type = struct.unpack(">B", header_log[8:9])[0]
                            # e_payload_type = struct.unpack(">B", header_log[9:10])[0]
                            # freq = struct.unpack(">H", header_log[10:12])[0]
                            # rssi = struct.unpack(">H", header_log[12:14])[0]
                            # is_signed = struct.unpack(">?", header_log[14:15])[0]
                            # reserved = header_log[15:16]
                            # peer_l2id = struct.unpack(">I", header_log[16:20])[0]
                            # reserved2 = header_log[20:24]
                            # reserved3 = header_log[24:32]
                            # crc = struct.unpack(">I", header_log[32:36])[0]
                            # length = struct.unpack(">H", header_log[36:38])[0]
                            ...
                            # ulTimeStamp = struct.unpack(">q", header_log[0][50:58])[0]
                    ...
                    # self.info_box.append(dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + "\n - Saving Log File\n(" + file_name + ")\n - Mileage : " + str(mileage_log))
        ...
        while wes_tag:
            try:
                ...
                if ws_resp[0:2] != WS_RESP_MAGIC_NUM:
                    print("Fail the WS Req/Resp process")
                    continue
                wes_tag = False
                break
            except:
                print(traceback.format_exc())
        print("Start receive worker thread")
                ...
                try:
                    if packet[packet_ptr:packet_ptr + 2] == RX_MAGIC_NUM:
                        if packet[packet_ptr+38:packet_ptr+40] == PING_INDICATOR:
                            ...
                            # print(receiver_send_time, receive_time, sender_recv_time, sender_send_time)
                            ...
                            if len(packet[packet_ptr:]) < 38 + 54 + payload_length:
                                ...
                                # print("pre : %d" %len(packet[packet_ptr:]))
                                break
    ...
    def __init__(self):
        ...
        while True:
            try:
                ...
                print("Receiver successfully connected to server")
                break
            except:
                print(traceback.format_exc())
    ...
    def __init__(self, label):
        ...
        while True:
            try:
                ...
                print(traceback.format_exc())
        print("Start Weather Condition worker thread")
    ...
    def run(self):
        ...
        while self.trig:
            try:
                ...
                print("Request Error & Rerequest weather")
                continue
    ...
    def __init__(self, label):
        ...
        while True:
            try:
                ...
                print(traceback.format_exc())
        print("Start Road Condition worker thread")
        ...
        while self.trig:
            try:
                ...
                try:
                    ...
                    print("Request Error & Rerequest road")
                    continue
...
class NavigationWindow(QWidget):
    def __init__(self):
        ...
        self.timer.start(1000)  # 1초마다 함수 실행
